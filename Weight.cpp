...
void Weight::setWeight( const Weight::t_weight newWeight ) {
    ...
    if( !isWeightValid( newWeight ) ) {
        throw out_of_range( "Weight not in range" ) ;
    }
...
void Weight::setMaxWeight( const Weight::t_weight newMaxWeight ) {
    ...
    if( !isWeightValid( newMaxWeight) ) {
        throw out_of_range( "The maximum weight is not in range" );
    }
...
bool Weight::isWeightValid( const Weight::t_weight checkWeight ) const noexcept {
    if( checkWeight <= 0 ) {
        cout << "Weight [" << checkWeight << "] cannot be less than or equal to 0" << endl ;
        return false;
    }

    if( bHasMax ) {
        if( checkWeight > maxWeight ) {
            cout << "Weight [" << checkWeight << "] is greater than the maximum weight [" << maxWeight << "]" << endl ;
            return false;
        }
    }
...
bool Weight::validate() const noexcept {
    if( bHasMax ) {
        if( !isWeightValid(( maxWeight))) {
            cout << "Max weight is less than or equal to 0" << endl;
            return false;
        }
        assert( maxWeight > 0 );
    }
